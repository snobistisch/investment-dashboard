...
// than a missing one. So every non-US venue suffix is
...
//
// One trap measured on 2026-08-09 lives in this file:
 ...
 *
 *  Windows are anchored on calendar days. Taking the last close on or before
 *  that date keeps the meaning stable across venues and holiday calendars.
 *
...
/** Last value on or before `date`. Markets and FX desks keep different
...
function computeReturns(series: Series): Returns | undefined {
  ...
  for (const [key, days] of RETURN_WINDOWS) {
    ...
    // Provider windows can stop just inside the requested boundary. Beyond
