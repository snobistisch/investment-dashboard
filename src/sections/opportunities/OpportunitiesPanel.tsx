...
export function OpportunitiesPanel() {
  ...
  const themeByTicker = useMemo(
    () => Object.fromEntries(positions.map((position) => [position.ticker, position.sections.join(' · ')])),
    [],
  )
  const equityLongs = useMemo(
    () => positions.filter((position) => position.stance === 'long').sort((a, b) => a.ticker.localeCompare(b.ticker)),
    [],
  )
  ...
  return (
    ...
    >
      ...
      <div className="mt-4"><EquityChartAtlas positions={positions} snapshot={market.snapshot} /></div>
    </Section>
  )
}
