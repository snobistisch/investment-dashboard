...
const currentTickers = new Set(equityOpportunityModels.map((model) => model.ticker))
const snapshots = existing.snapshots.filter(
  (row) => currentTickers.has(row.ticker) && !replacements.has(`${row.asOf}|${row.ticker}|${row.modelVersion}`),
)
