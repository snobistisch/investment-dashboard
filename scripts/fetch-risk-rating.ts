...
/** Crypto trades every day. Using the equity convention sqrt(252) understates
 *  realised volatility by about 17%, and makes this rating inconsistent with
 *  the active-volatility calculation in build-portfolio.ts. */
...
function realisedVolPct(closes: number[]) {
  ...
  return Math.sqrt(variance) * Math.sqrt(DAYS_PER_YEAR) * 100
}
...
async function main() {
   ...
   *  forty network calls. This is also the reproducibility path: a formula
   *  change must not silently change the underlying market-data vintage. */
  const reuseHistory = process.argv.includes('--from-cache')
  const cached = reuseHistory
    ? (JSON.parse(readFileSync(OUT_HISTORY, 'utf8')) as {
        fetchedAt: string
        series: Record<string, { closes: { d: string; c: number }[] }>
      })
    : undefined
  ...
  for (const token of tokens) {
    ...
    try {
      ...
      const series = cachedSeries ?? (await cgHistory(id))
    ...
    if (!reuseHistory) await new Promise((r) => setTimeout(r, CG_SPACING_MS))
  }
  ...
  const body =
    ...
    `   inside the window. Fetched ${dataFetchedAt.slice(0, 10)}.\n` +
